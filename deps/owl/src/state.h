...
#define RSSI_THRESHOLD_DEFAULT -80
...
struct awdl_state {
	...
	struct awdl_rx_clock rx_clock;
	struct awdl_channel_state channel;
	struct awdl_peer_state peers;
	struct awdl_stats stats;
	/* Track the sync master's channel sequence instead of a fixed one. */
	uint8_t follow_master_chanseq : 1;
	/* The sequence configured at startup, restored when no master is followed. */
	struct awdl_chan configured_sequence[AWDL_CHANSEQ_LENGTH];
	/* Follow this peer's sequence rather than the elected master's, if set. */
	struct ether_addr preferred_peer;
	uint8_t have_preferred_peer : 1;
	/* Stable within one radio session; changed when the session is replaced. */
	uint16_t service_update_indicator;
};
...
struct ieee80211_state {
	int qos_data; /* Optional QoS data framing for interoperability tests. */
