...
int awdl_handle_version_tlv(struct awdl_peer *src, const struct buf *val,
	...
	if (src->version != version) {
		log_debug("peer %s runs AWDL %u.%u (devclass %u), we advertise %u.%u",
		          ether_ntoa(&src->addr), (version >> 4) & 0xf, version & 0xf,
		          devclass, (state->version >> 4) & 0xf, state->version & 0xf);
	}
	src->version = version;
...
int awdl_rx_data(const struct buf *frame, struct buf ***out, const struct ether_addr *src,
	...
	/* The host consumes buf_len(), so allocate exactly the Ethernet frame.
	 * A maximum-size buffer would append unwritten bytes to every packet. */
	**out = buf_new_owned(sizeof(struct ether_header) + buf_len(frame));
...
int awdl_rx(const struct buf *frame, struct buf ***data_frame, struct awdl_state *state) {
	...
	signed char rssi = 0;
	uint64_t tsft, hardware_tsft = 0;
	uint8_t flags = 0; /* Radiotap fields are optional. */

	tsft = clock_time_us();
	if (radiotap_parse(frame, &rssi, &flags, &hardware_tsft) < 0)
		return RX_UNEXPECTED_FORMAT;
#ifdef __APPLE__
	uint64_t received = awdl_rx_clock_time(&state->rx_clock, hardware_tsft, tsft);
	if (tsft - received > 3000)
		log_trace("corrected receive delivery delay %llu us", (unsigned long long)(tsft - received));
	tsft = received;
#endif
