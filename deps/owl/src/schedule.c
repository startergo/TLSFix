...
bool awdl_on_scheduled_channel(const struct awdl_state *state, uint64_t now) {
    int slot = awdl_sync_current_eaw(now, &state->sync) % AWDL_CHANSEQ_LENGTH;
    int planned = awdl_chan_num(state->channel.sequence[slot], state->channel.enc);
    return planned && planned == awdl_chan_num(state->channel.current, state->channel.enc);
}
...
int awdl_is_multicast_eaw(const struct awdl_state *state, uint64_t now) {
	...
	/*
	 * Slot 8 carries the 2.4GHz social channel in every sequence observed from
	 * current devices, and is often the only slot a peer spends there. Limiting
	 * multicast to slots 0 and 10 puts every announcement and query response on
	 * a 5GHz channel, so a peer that only listens on the social channel never
	 * hears us answer its browse.
	 */
	return slot == 0 || slot == 8 || slot == 10;
}
...
double awdl_can_send_multicast_in(const struct awdl_state *state, uint64_t now, int guard) {
    if (!awdl_is_multicast_eaw(state, now))
        return usec_to_sec(awdl_sync_next_aw_us(now, &state->sync));
    /* A channel timer and a transmit timer can become runnable together.
     * The schedule alone does not mean the radio has finished retuning. */
    if (!awdl_on_scheduled_channel(state, now))
        return 0.001;
    return awdl_can_send_in(state, now, guard);
}
...
double awdl_can_send_unicast_in(const struct awdl_state *state, const struct awdl_peer *peer, uint64_t now, int guard) {
	...
	/* A matching pair of advertised schedules does not prove the radio has
     * finished retuning. Never inject on the previous slot's channel. */
    if (!awdl_on_scheduled_channel(state, now))
        return 0.001;

    if (next_aw < _guard) { /* we are at the end of slot */
		/* Agreement with the peer in the adjacent slot is insufficient:
		 * both radios may be hopping to a different channel. Only waive
		 * the guard when that slot also keeps our current radio channel. */
		if (awdl_on_scheduled_channel(state, now + eaw) &&
		    awdl_same_channel_as_peer(state, now + eaw, peer)) {
