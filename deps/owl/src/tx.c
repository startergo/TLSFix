...
int awdl_init_service_params_tlv(uint8_t *buf, const struct awdl_state *state) {
	...
	tlv->unknown[1] = 0;
	tlv->unknown[2] = 0;
	tlv->sui = htole16(state->service_update_indicator);
...
int ieee80211_init_awdl_data_hdr(uint8_t *buf, const struct ether_addr *src, const struct ether_addr *dst,
                                 struct ieee80211_state *state) {
	uint16_t type = IEEE80211_FTYPE_DATA | IEEE80211_STYPE_DATA;
	if (state->qos_data)
		type |= IEEE80211_STYPE_QOS_DATA;
	int length = ieee80211_init_awdl_hdr(buf, src, dst, state, type);
	if (state->qos_data) {
		/* Best effort TID, normal ACK policy, no aggregation. */
		memset(buf + length, 0, IEEE80211_QOS_CTL_LEN);
		length += IEEE80211_QOS_CTL_LEN;
	}
	return length;
}
