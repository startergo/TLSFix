...
static int open_tun(char *dev, const struct ether_addr *self) {
	...
	for (int i = 0; i < 16; ++i) {
		...
		if (fd > 0) {
			...
			/* Set reduced MTU */
			ifr.ifr_mtu = OWL_DARWIN_TAP_MTU;
...
int wlan_send(const struct io_state *state, const uint8_t *buf, int len) {
	...
	if (!state || !state->wlan_handle)
		return -EINVAL;
	/*
	 * Frames arrive here already carrying a radiotap header, built by
	 * ieee80211_init_radiotap_header() in awdl_init_full_action_frame() and
	 * awdl_init_full_data_frame(). Adding another one here corrupts every
	 * transmission.
	 */
