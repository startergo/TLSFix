...
/*
 * Appends a frame to the capture file.
 *
 * libpcap 1.3.0, which is what Mac OS X 10.9 ships, has no
 * pcap_dump_open_append(). The dumper is therefore opened once and held for the
 * lifetime of the process instead of being reopened per frame; each frame is
 * flushed so the capture stays readable while owl is running. The file is
 * truncated when first written rather than appended to across runs.
 */
static void dump_frame(const char *dump_file, const struct pcap_pkthdr *hdr, const uint8_t *buf) {
	...
	if (!dumper) {
		p = pcap_open_dead(DLT_IEEE802_11_RADIO, 65535);
		if (!p) {
			log_error("pcap: could not allocate dump handle");
			return;
		}
		dumper = pcap_dump_open(p, dump_file);
		if (!dumper) {
			log_error("pcap: could not open dump file %s (%s)", dump_file, pcap_geterr(p));
			pcap_close(p);
			p = NULL;
			return;
		}
...
static void adopt_master_chanseq(struct awdl_state *state);
...
#define MAX_NUM_AMPDU 16 /* TODO lookup this constant from the standard */
	...
	if (!state->io.wlan_is_file) {
		...
		int64_t age = ((int64_t) delivered.tv_sec - hdr->ts.tv_sec) * 1000000
		              + delivered.tv_usec - hdr->ts.tv_usec;
		if (state->telemetry && age > state->diag_rx_age_max) state->diag_rx_age_max = age;
        if ((samples++ % 100) == 0)
			log_debug("capture delivery age %lld us", (long long) age);
	}
#endif
	int result;
	...
	uint64_t actions_before = state->awdl_state.stats.rx_action;
	result = awdl_rx(frame, &data, &state->awdl_state);
	if (result == RX_OK) {
		/* A peer can change to a sparse low-power schedule between cleanup
		 * ticks. Publish and follow that schedule before the next action/data
		 * callback, rather than advertising the old slots for up to a second.
		 * Keep election and expiry on their existing maintenance timer. */
		if (state->awdl_state.stats.rx_action != actions_before)
			adopt_master_chanseq(&state->awdl_state);
...
int awdl_send_data(const struct buf *buf, const struct io_state *io_state,
	...
	log_trace("Send data (len %d) to %s (%u.%u.%u) channel current=%u planned=%u", awdl_data_len,
	          ether_ntoa(&dst), period, slot, tu,
	          awdl_chan_num(awdl_state->channel.current, awdl_state->channel.enc),
	          awdl_chan_num(awdl_state->channel.sequence[slot], awdl_state->channel.enc));
...
void awdl_send_unicast(struct ev_loop *loop, ev_timer *timer, int revents) {
	...
	if (state->next) { /* we have something to send */
		...
		if (compare_ether_addr(&dst, &awdl_state->self_address) == 0) {
			...
			/* Timers can run after a host-read callback, and adopting a new
             * sequence can change the planned channel between channel ticks.
             * Retune before injection, then recheck time/guard after the call. */
            if (awdl_same_channel_as_peer(awdl_state, now, peer) && !awdl_on_scheduled_channel(awdl_state, now)) {
                awdl_switch_channel(loop, &state->ev_state.chan_timer, 0);
                now = clock_time_us();
            }
            in = awdl_can_send_unicast_in(awdl_state, peer, now, AWDL_UNICAST_GUARD_TU);
			if (in == 0) { /* send now */
				int result = awdl_send_data(state->next, &state->io, &state->awdl_state, &state->ieee80211_state);
                state->diag_tx++;
                if (result != TX_OK) state->diag_fail++;
                int slot = awdl_sync_current_eaw(now, &awdl_state->sync) % AWDL_CHANSEQ_LENGTH;
                if (awdl_chan_num(awdl_state->channel.current, awdl_state->channel.enc) !=
                    awdl_chan_num(awdl_state->channel.sequence[slot], awdl_state->channel.enc)) state->diag_mismatch++;
				buf_free(state->next);
				state->next = NULL;
...
void awdl_send_multicast(struct ev_loop *loop, ev_timer *timer, int revents) {
	...
	if (!circular_buf_empty(state->tx_queue_multicast)) { /* we have something to send */
		in = awdl_can_send_multicast_in(awdl_state, now, AWDL_MULTICAST_GUARD_TU);
...
void awdl_switch_channel(struct ev_loop *loop, ev_timer *timer, int revents) {
	...
	/* CoreWLAN can retune the card outside this event loop. A cached channel
	 * must not strand a static sequence indefinitely after that happens. */
#ifdef __APPLE__
	if (!state->io.wlan_is_file && now - state->channel_checked_at >= 1000000) {
		state->channel_checked_at = now;
		int actual = corewlan_get_channel(state->io.wlan_ifindex);
		if (actual > 0 && actual != chan_num_old) {
			log_warn("radio channel changed outside OWL: expected %d, actual %d", chan_num_old, actual);
			chan_num_old = actual;
		}
	}
#endif
	...
	if (chan_num_new && (chan_num_new != chan_num_old)) {
		...
		if (!state->io.wlan_is_file) {
			...
			int selected = set_channel(state->io.wlan_ifindex, chan_num_new);
#ifdef __APPLE__
			if (selected <= 0) goto rearm_channel;
#else
			if (selected < 0) goto rearm_channel;
#endif
			uint64_t elapsed = clock_time_us() - now;
            state->diag_switches++; state->diag_switch_us += elapsed;
            if (elapsed > state->diag_switch_max_us) state->diag_switch_max_us = elapsed;
#ifdef __APPLE__
            if (state->telemetry && corewlan_get_channel(state->io.wlan_ifindex) != chan_num_new) state->diag_actual_wrong++;
#endif
            log_debug("channel switch took %llu us", (unsigned long long)elapsed);
		}
		awdl_state->channel.current = chan_new;
	}

rearm_channel:
	next_aw = awdl_sync_next_aw_us(clock_time_us(), &awdl_state->sync);
...
/*
 * Copies the sync master's channel sequence over our own.
 *
 * With a fixed sequence we sit on one channel while the master hops, and
 * unicast can only go out in the slots where the two happen to coincide - as
 * little as one slot in sixteen against a real device, which is not enough to
 * carry a TCP connection. Following the master is what AWDL expects of a
 * non-master node and puts us on the right channel for every one of its active
 * slots.
 */
/* Puts our own configured channel sequence back after following a master. */
static void restore_configured_chanseq(struct awdl_state *state) {
	if (!memcmp(state->channel.sequence, state->configured_sequence,
	            sizeof(state->channel.sequence)))
		return;

	memcpy(state->channel.sequence, state->configured_sequence,
	       sizeof(state->channel.sequence));
	log_debug("restored configured channel sequence");
}
...
static void adopt_master_chanseq(struct awdl_state *state) {
	...
	if (!state->follow_master_chanseq)
		return;

	/*
	 * With several Apple devices in range the election can settle on one that
	 * shares no channel with the device we actually want to reach, which parks
	 * us on a frequency it never visits. A preferred peer pins us to the one
	 * that matters.
	 */
	if (state->have_preferred_peer) {
		if (awdl_peer_get(state->peers.peers, &state->preferred_peer, &master) == PEERS_OK) {
			int i, n = 0;

			/*
			 * Only the channel sequence is taken from the preferred peer.
			 * Overriding election.sync_addr as well was tried and measured
			 * worse: it is recomputed by awdl_election_run() every second, so
			 * forcing it fights the election and corrupts the election state we
			 * advertise to peers.
			 */
			for (i = 0; i < AWDL_CHANSEQ_LENGTH; i++) {
				if (awdl_chan_num(master->sequence[i], state->channel.enc))
					n++;
			}
			if (n && memcmp(state->channel.sequence, master->sequence,
			                sizeof(state->channel.sequence))) {
				memcpy(state->channel.sequence, master->sequence,
				       sizeof(state->channel.sequence));
				log_debug("following preferred peer %s (%d active slots)",
				          ether_ntoa(&state->preferred_peer), n);
			}
			return;
		}
		/* Preferred peer not present; fall through to normal behaviour. */
	}

	/*
	 * As sync master our own sequence is the one everyone else follows. The
	 * same applies when the master disappears: keeping its sequence would strand
	 * us on whatever channel it favoured, off the social channel where peers are
	 * found, and no new peer could ever be discovered to recover from it.
	 */
	if (awdl_election_is_sync_master(&state->election, &state->self_address)) {
		restore_configured_chanseq(state);
		return;
	}

	if (awdl_peer_get(state->peers.peers, &state->election.sync_addr, &master) != PEERS_OK) {
		restore_configured_chanseq(state);
		return;
	}

	/* A sequence of all-null slots would leave us unable to transmit at all. */
	for (i = 0; i < AWDL_CHANSEQ_LENGTH; i++) {
		if (awdl_chan_num(master->sequence[i], state->channel.enc))
			active++;
	}
	...
	if (!memcmp(state->channel.sequence, master->sequence, sizeof(state->channel.sequence)))
		return; /* already following it */

	memcpy(state->channel.sequence, master->sequence, sizeof(state->channel.sequence));

	{
		char seq[AWDL_CHANSEQ_LENGTH * 5 + 1];
		int off = 0;
		for (i = 0; i < AWDL_CHANSEQ_LENGTH; i++) {
			off += snprintf(seq + off, sizeof(seq) - off, "%s%d", i ? "," : "",
			                awdl_chan_num(master->sequence[i], state->channel.enc));
		}
		log_debug("adopted channel sequence from master %s: %s (%d active slots)",
		          ether_ntoa(&state->election.sync_addr), seq, active);
	}
}
